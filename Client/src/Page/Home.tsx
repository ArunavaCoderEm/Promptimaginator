    ...
    <>
      <div className="w-full my-10 h-screen grid lg:grid-cols-2 sm:grid-cols-1">    
        ...
        <div className="flex flex-col my-10 justify-center h-[70vh] m-auto w-full">
          ...
          <form className="p-2 w-full mx-auto flex flex-col lg:flex-row gap-4" onSubmit={submithandle}>
            <input
              onChange={(e: any) => {
                setinp(e.target.value);
              }}
              value={inp}
              className="w-full p-2 text-lg font-thin h-16 rounded-lg backdrop-blur-sm bg-white/10 border-gray-400 placeholder:text-pink-600 focus:border-none focus:outline-none text-center text-pink-700"
              required
              placeholder="Enter Image Prompt Here ..."
            />
            <button
              type="submit"
              className="w-24 lg:mr-10 sm:mr-0 h-16 mb-10 mx-auto px-2 rounded-md font-semibold text-white bg-pink-500 hover:scale-95 hover:bg-pink-700 transition-all duration-300"
            >
              Generate
            </button>
          </form>
      ...
      :
      <>
      <div className="grid mt-10 bg-black lg:grid-cols-2 sm:grid-cols-1 gap-4">
        {img.map((item, index) => (
            <div key={index}>
            <Imgcard 
            img={item}
